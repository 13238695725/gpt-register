...
class SkymailProvider(DomainAwareMailProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self.domain = self.domains[0] if self.domains else ""
        if not self.api_base:
            raise RuntimeError("skymail.api_base 未配置，无法创建 SkyMail 邮箱。")
        if not self.domains:
            raise RuntimeError("skymail.domains 未配置，无法创建 SkyMail 邮箱。")
        # 如果没有提供token，尝试生成token
        if not self.token and self.admin_email and self.admin_password:
    ...
    def _generate_token(self) -> str:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if token:
                    self.logger.info("SkyMail token 生成成功")
                    return str(token)
            self.logger.error("SkyMail token 生成失败: status=%s body=%s", resp.status_code, resp.text[:200])
        except Exception as exc:
            self.logger.error("SkyMail token 生成异常: %s", exc)
        return ""

    def _create_address_for_domain(self, domain: str) -> Optional[Mailbox]:
        if not self.token:
            self.logger.error("SkyMail token 不可用，无法创建邮箱")
            return None
        ...
        try:
            resp = session.post(
                ...
                json={
                    "list": [{
                        "email": email,
                        "password": secrets.token_hex(8)  # 随机生成密码
                    }]
                },
                timeout=15,
            )
            if resp.status_code == 200:
                self.logger.info("SkyMail 创建邮箱成功: %s", email)
                return Mailbox(
                    email=email,
                    token=self.token,  # 使用相同的token来查询邮件
                    domain=domain,
                    failure_target=domain,
                )
            else:
                self.logger.warning("SkyMail 创建邮箱失败: domain=%s status=%s body=%s", domain, resp.status_code, resp.text[:200])
        except Exception as exc:
            self.logger.warning("SkyMail 创建邮箱异常: domain=%s error=%s", domain, exc)
        return None
    ...
    def _fetch_skymail_messages(self, mailbox: Mailbox) -> List[Dict[str, Any]]:
        if not self.token:
            return []
        session = self._session()
        try:
            ...
            if resp.status_code == 200:
                ...
                self.logger.warning("SkyMail 查询邮件失败: status=%s body=%s", resp.status_code, resp.text[:200])
        except Exception as exc:
            self.logger.warning("SkyMail 查询邮件异常: %s", exc)
        return []
    ...
    def poll_verification_codes(
        ...
        *,
        ...
        messages = self._fetch_skymail_messages(mailbox)
        codes: List[str] = []
        normalized_email = (email or mailbox.email).strip().lower()
        for message in messages:
            if not isinstance(message, dict):
            ...
            if create_time:
                try:
                    # 假设时间格式为 "2099-12-30 23:59:59"
                    message_ts = datetime.strptime(create_time, "%Y-%m-%d %H:%M:%S").timestamp()
                    if not_before_ts and message_ts < not_before_ts:
                        continue
                except:
                    pass  # 如果时间解析失败，继续处理

            recipient = str(message.get("toEmail") or "").strip().lower()
            if recipient and normalized_email and recipient != normalized_email:
                continue
            
            # 从邮件内容中提取验证码
            content = str(message.get("content") or "") + " " + str(message.get("text") or "")
            subject = str(message.get("subject") or "")
            full_content = f"{subject}\n{content}"
            
            if "openai" not in full_content.lower() and "chatgpt" not in full_content.lower():
                continue
...
def build_mail_provider(conf: Dict[str, Any], proxy: str, logger: logging.Logger) -> MailProviderBase:
    ...
    if not raw_provider:
        if pick_conf(conf, "mail", "api_base", default=conf.get("mail_api_base")) or pick_conf(
            ...
            provider_name = "yyds_mail"
        elif pick_conf(conf, "skymail", "token", default=conf.get("skymail_token")) or pick_conf(
            conf, "skymail", "admin_email", default=conf.get("skymail_admin_email")
        ):
            provider_name = "skymail"
    ...
    if provider_name == "skymail":
        return SkymailProvider(
            proxy=proxy,
            logger=logger,
            api_base=str(pick_conf(conf, "skymail", "api_base", default=conf.get("skymail_api_base", "")) or "").strip(),
            token=str(pick_conf(conf, "skymail", "token", default=conf.get("skymail_token", "")) or "").strip(),
            admin_email=str(pick_conf(conf, "skymail", "admin_email", default=conf.get("skymail_admin_email", "")) or "").strip(),
            admin_password=str(pick_conf(conf, "skymail", "admin_password", default=conf.get("skymail_admin_password", "")) or "").strip(),
            domain=str(pick_conf(conf, "skymail", "domain", default=conf.get("skymail_domain", "")) or "").strip(),
            domains=pick_conf_list(conf, "skymail", "domains", "skymail_domains"),
            failure_threshold=failure_threshold,
            failure_cooldown_seconds=failure_cooldown_seconds,
        )
