...
- Docker Compose 服务器部署入口（`docker-compose.yml` + `install_server.sh`）

---

## 上传到 GitHub

这个目录当前还不是一个 Git 仓库的话，先在项目根目录执行：
...
- 仓库里已经建议忽略 `.env`、`config.json`、`docker-data/`、`logs/`、`admin_token.txt` 等敏感或运行时文件
- 正式上传前，确认不要把真实的 `config.json`、token、日志一起推上去
...
- 不需要手动装 Python 虚拟环境和 Node 运行环境
- 前后端一起启动，适合云服务器长期运行
- 更新时只需要 `git pull` + `docker compose up -d --build`

### 方式一：先 `git clone`，再执行安装脚本
...
```bash
curl -fsSL https://raw.githubusercontent.com/你的用户名/你的仓库名/main/install_server.sh | bash -s -- \
  --repo https://github.com/你的用户名/你的仓库名.git \
  --dir /opt/gpt-register-oss
```

脚本会自动完成这些事：

1. 在 Debian / Ubuntu 上安装 `git`、`docker`、`docker compose`
2. 克隆或更新仓库
3. 生成 `.env`
4. 生成 `docker-data/backend/config.json`
5. 执行 `docker compose up -d --build`

部署后：

- 前端地址：`http://你的服务器IP:8080`
- 后端容器内 API：`http://backend:8318`
- 首次启动生成的管理 token 文件：`docker-data/backend/admin_token.txt`

### 服务器更新命令

```bash
cd /opt/gpt-register-oss
git pull --ff-only
